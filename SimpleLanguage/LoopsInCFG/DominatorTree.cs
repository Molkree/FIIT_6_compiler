...
namespace SimpleLanguage
{
    ...
    public class DominatorTree : GenericIterativeAlgorithm<IEnumerable<BasicBlock>>
    {
        ...
        public class Tree
        {
            ...
            public IReadOnlyCollection<BasicBlock> Children(BasicBlock node) => !nodes.Contains(node)
                    ? throw new ArgumentException("The tree doesn't contain the node")
                    : children.TryGetValue(node, out var result) ? result : new List<BasicBlock>();
            ...
            public BasicBlock Parent(BasicBlock node) => !nodes.Contains(node)
                    ? throw new ArgumentException("The tree doesn't contain the node")
                    : parents.TryGetValue(node, out var result) ? result : null;
